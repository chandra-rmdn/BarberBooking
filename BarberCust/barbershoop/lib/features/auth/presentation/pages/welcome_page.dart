...
import 'reservation_page.dart'; // <--- 2. BIAR BISA PINDAH KE RESERVATION PAGE
  ...
  void initState() {
    ...
    Future.delayed(const Duration(seconds: 2), () async {
      final user = await FirebaseAuth.instance
          .authStateChanges()
          .first; // Ambil user saat ini (jika ada)

      if (!mounted) return; // Pastikan widget masih ada di tree sebelum navigasi

      if (user != null) {
        // Jika user sudah login, langsung ke halaman utama (misalnya HomePage)
        Navigator.pushReplacement(
          context,
          MaterialPageRoute(builder: (context) => const ReservationPage()), // Ganti dengan HomePage jika ada
        );
      } else {
          ...
          SafeArea(
            child: Column(
              ...
              children: [
                    ...
                    RichText(
                      text: const TextSpan(
                        ...
                        children: [
                          ...
                          TextSpan(
                            text: 'Barbershop',
                            style: TextStyle(color: Color(0xFF4682B4), fontWeight: FontWeight.w500), 
                          ),
                        ],
                      ),
                    ),
                ...
                const Padding(
                  padding: EdgeInsets.only(bottom: 60.0, left: 32.0, right: 32.0),
                  child: Text(
